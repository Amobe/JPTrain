...
loadJSON('common/data/hiragana.json', function (obj) {
  ...
  for (var table in tables) {
    if (tables.hasOwnProperty(table)) hiraganaTables[table] = createTable(tables[table]);
  }
});
loadJSON('common/data/katakana.json', function (obj) {
  ...
  for (var table in tables) {
    if (tables.hasOwnProperty(table)) katakanaTables[table] = createTable(tables[table]);
  }
});
